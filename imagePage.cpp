  ...
  QWizardPage (parent)
{
  ...
  QRadioButton *lgs = new QRadioButton (tr ("Light Gray Scale"));
  QRadioButton *mgs = new QRadioButton (tr ("Medium Gray Scale"));
  QRadioButton *r2b = new QRadioButton (tr ("Red To Blue"));
  QRadioButton *lrb = new QRadioButton (tr ("Light Red To Blue"));
  QRadioButton *r2m = new QRadioButton (tr ("Red To Magenta"));
  QRadioButton *m2g = new QRadioButton (tr ("Magenta To Green"));
  ...
  QGroupBox *fBox = new QGroupBox (tr ("Options"), this);
  ...
  QCheckBox *transparent_check = new QCheckBox (tBox);
  ...
  QCheckBox *caris_check = new QCheckBox (cBox);
  ...
  registerField ("restart_check", restart_check);
}
...
{
  float               row[2][SAMPLE_WIDTH], range[2] = {0.0, 0.0}, shade_factor;
  int32_t             c_index = 0, hue, sat;
  ...
  palshd (NUMSHADES, NUMHUES, (float) endSpin->value (), (float) startSpin->value (), 
          (float) satSpin->value (), (float) satSpin->value (), (float) valSpin->value (),
      ...
      for (int32_t j = 0 ; j < SAMPLE_WIDTH ; j++)
        {
          ...
          row[1][j] = (float) options->sample_data[i][j];
        }

      if (i && i < SAMPLE_HEIGHT)
        {
          for (int32_t k = 0 ; k < SAMPLE_WIDTH ; k++)
            {
              if (cross_zero)
                {
                  if (row[0][k] < 0.0)
                    {
                      c_index = (int32_t) (NUMHUES - (int32_t) (fabsf ((row[0][k] - options->sample_min) / range[0] * 
                                                                         NUMHUES))) * NUMSHADES;
                    }
                  else
                    {
                      c_index = (int32_t) (NUMHUES - (int32_t) (fabsf (row[0][k]) / range[1] * NUMHUES)) *
                        NUMSHADES;
                    }
                }
              else
                {
                  c_index = (int32_t) (NUMHUES - (int32_t) (fabsf ((row[0][k] - options->sample_min) / range[0] * NUMHUES))) *
                    NUMSHADES;
                }
